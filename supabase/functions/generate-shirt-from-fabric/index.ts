...
All text must be perfectly legible and correctly spelled — render ONLY the labels and values listed above. Absolutely NO rating, NO stars, NO review count, NO numeric chip anywhere in the image. CRITICAL BRAND CLEANUP: key highlights must contain NO collar tag, NO neck label, NO chest logo, NO monogram, NO watermark, NO brand wordmark, and NO floating brand mark anywhere. The shirt should look plain except for its fabric, seams, buttons, and pocket.`
  ...
  const common = `Photorealistic flat-lay studio product photograph of a men's full-sleeve button-down shirt on a pure white seamless background. The shirt MUST include one neat chest pocket on the wearer's LEFT side (viewer’s RIGHT side) in every front-facing output. Soft even lighting, no harsh shadows on background, perfectly centered, NO model, NO mannequin, NO hands, NO props, NO text overlays, NO watermark, NO printed graphics or large logos on the shirt body (a small tonal embroidered MUFFIGOUT logo on the pocket corner is allowed only when explicitly requested below). Front and back outputs must look like the exact same garment photographed from two sides, with identical color, fabric texture, pattern scale, seam style, sleeve/cuff design, and hem shape. ${quality} ${colorLock} ${patternLock} ${fabricFidelity}`
  if (view === 'front') {
    return `${common} VIEW: FRONT view. Shirt laid flat and perfectly symmetric on pure white seamless background, collar at top, full placket with buttons visible down the center, chest pocket on the wearer's LEFT chest (viewer's RIGHT side), both sleeves spread slightly outward, cuffs visible.

CRITICAL FRAMING (NON-NEGOTIABLE): Render the image in PORTRAIT 3:4 aspect ratio. The ENTIRE shirt — collar tip at top, BOTH cuffs at the sides, and the FULL curved hem at the bottom — MUST be completely inside the frame with at least 8-10% safe-margin whitespace on every side (top, bottom, left, right). NOTHING may touch or cross the image edge. Sleeves must NOT be cropped, cuffs must NOT be cut off, and the hem must NOT be clipped. Shrink the garment within the frame if needed to guarantee these margins. The composition should feel like a clean catalog product card where the whole shirt is visible at a glance.

The ENTIRE shirt — body, collar, collar stand, placket, sleeves, cuffs, and pocket — MUST be cut from the EXACT same fabric in the provided swatch image: identical hue, identical brightness, identical saturation, identical pattern (stripe spacing / check size / motif scale / weave direction). Do NOT shift the color toward grey, blue, beige, or any other tone. Do NOT change stripe width, stripe color, stripe count, or pattern direction. A side-by-side of the swatch and the rendered shirt fabric MUST be indistinguishable.

CRITICAL COLLAR LABEL (brand neck tag — MANDATORY, must be clearly visible):
- Position: centered on the INSIDE BACK NECKBAND, on the inner facing of the collar stand, fully visible because the collar is laid flat showing the inside of the neck.
- Shape: small neat horizontal rectangle, width roughly 22-28% of the collar width, height roughly 35-40% of its own width, with tiny stitched edges on the left and right short sides (sewn-in woven tag look — NOT a printed sticker, NOT a hang tag).
- Tag color: if the shirt is light/medium tone, use a DARK NAVY or BLACK woven tag with CREAM/IVORY text. If the shirt is dark, use a CREAM/IVORY tag with DARK NAVY text. The tag must clearly CONTRAST with the shirt fabric so it reads instantly.
- Text: the single brand wordmark "MUFFIGOUT" in clean bold uppercase sans-serif, perfectly legible, correctly spelled letter-by-letter M-U-F-F-I-G-O-U-T (10 letters total, no missing letter, no extra letter, no other word, no tagline, no size, no care symbols, no second tag).
- Only ONE collar label. No other label, tag, or text anywhere else on the shirt.

CRITICAL POCKET EMBROIDERY (MG monogram — MANDATORY, must be clearly visible on the pocket):
- Position: on the TOP-LEFT CORNER of the chest pocket (wearer's LEFT chest / viewer's RIGHT side of the image), placed just inside the upper-left corner of the pocket with a small clean inset margin (roughly 6-8% of pocket width from each edge).
- Mark: a small stitched embroidered "MG" monogram (two uppercase letters M and G, slightly overlapping or paired in an elegant serif/condensed-sans logotype) — NOT the full word "MUFFIGOUT", just the two-letter "MG" mark.
- Size: roughly 18-22% of the pocket width — small but clearly visible at thumbnail size.
- Thread color: stitched in a SLIGHTLY DARKER tonal shade of the EXACT shirt/fabric color (or slightly lighter tonal shade only if the shirt is very dark). It must look like real embroidered thread with visible stitch texture — tone-on-tone, subtle, premium. NEVER bright white, black, gold, red, or any contrasting/off-color thread. NEVER a printed sticker or flat graphic.
- Only ONE embroidery mark, only on the pocket corner. No logo on the chest, no logo on the sleeve, no logo on the placket, no second mark anywhere.`
  }
  if (view === 'back') {
    return `${common} VIEW: BACK view. Shirt laid flat and perfectly symmetric, back yoke visible at the shoulders, no buttons visible, smooth uninterrupted back panel, both sleeves spread slightly outward. CRITICAL FRAMING: render in PORTRAIT 3:4 aspect with at least 8-10% safe-margin whitespace on every side — the whole shirt (collar, both cuffs, full hem) must be fully inside the frame and NEVER cropped. The back view MUST match the front reference garment exactly in color, pattern density, sleeve angle, collar construction, cuff design, fabric brightness, and scale. CRITICAL: do NOT render any collar tag, neck label, sewn-in label, brand tag, woven tag, hang tag, sticker, monogram, embroidery, printed text, or any visible label anywhere on the back of the shirt — the entire back, including the inside back-collar/neckband area, must be completely clean fabric only. NO tag, NO label of any kind.`
  }
